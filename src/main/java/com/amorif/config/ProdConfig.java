...
//		Create Roles
		...
		if (regraRepository.count() == 0) {
			...
			List<Regra> regras = Arrays.asList(
					...
					Regra.builder().descricao("0 pontos pela limpeza ruim").operacao("SUM").valorMinimo(0)
							.senso(limpeza).tipoRegra(tipoFixo).roles(Arrays.asList(apoioAcademico, administrador))
							.build(),
					Regra.builder().descricao("5 pontos pela limpeza mediana").operacao("SUM").valorMinimo(5)
							.senso(limpeza).tipoRegra(tipoFixo).roles(Arrays.asList(apoioAcademico, administrador))
							.build(),
					...
					Regra.builder()
							.descricao("10 pontos para a turma que mais participou de CAs no bimestre (opcional)")
					...
					Regra.builder().descricao("1 ponto por dia de aluno da turma suspenso").operacao("SUB")
							.valorMinimo(1).senso(autodisciplina).tipoRegra(tipoVariavel)
							.roles(Arrays.asList(apoioAcademico, administrador)).build(),
